<form id="currentPageInfo_<?=$current_object;?>">
	<input type="hidden" name="category" value="<?=$current_category;?>" />
	<input type="hidden" name="module" value="<?=$current_module;?>" />
	<input type="hidden" name="id" value="<?=$id;?>" />
	<input type="hidden" id="current_page_<?=$current_object;?>" name="page" value="<?=$current_page;?>" />
	<input type="hidden" id="current_order_<?=$current_object;?>" name="order" value="<?=$current_order;?>" />
	<input type="hidden" id="current_order_<?=$current_object;?>sens" name="order_sens" value="<?=$current_order_sens;?>" />
</form>
			...
			<table cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					...
					if(count($page_objects) > 0){
						...
						<tr>
							...
							<td colspan="<?=count($fields);?>" >
								...
								foreach($mass as $id => $val){?>
									<span class="mass_operation" object="<?=$current_object;?>" rel="<?=$id;?>"><?=$val['title'];?></span>
								<?}?>
							</td>
						</tr>
						<? }
						...
						?>
						<tr>
							<td style="text-align:center" colspan="<?=count($fields)+1;?>">
								Aucun résultat
							</td>
						</tr>
						<?
...
<div style="float:left">
	<div class="filter_box">
		<form id="formFilter_<?=$current_object;?>">
...
<? if($total_pages > 1) { ?>
	<div class="pagination">
		...
		<ul class="pag_list">
			<? if($current_page > 1){ ?>
			<li>
				<a href="javascript:void(0)" rel="<?=$current_page-1;?>" object="<?=$current_object;?>" class="pag_nav list_pg">
					<span>
						<span>Précédent</span>
					</span>
				</a>
			...
			if($min_pagination>1){
				?>
				<li><a href="javascript:void(0)" rel="1" object="<?=$current_object;?>" class="list_pg">1</a></li>
				<?
			}
			...
			for($i=$min_pagination;$i<=$max_pagination;$i++){
				if($current_page == $i){
					?>
					<li><a href="javascript:void(0)" rel="<?=$i;?>" object="<?=$current_object;?>" class="current_page list_pg"><span><span><?=$i;?></span></span></a></li>
					<?
				} else {
					?>
					<li><a href="javascript:void(0)" rel="<?=$i;?>" object="<?=$current_object;?>" class="list_pg"><?=$i;?></a></li>
					<?
				}
			}
			...
			if($max_pagination < $total_pages){
				?>
				<li><a rel="<?=$total_pages;?>" class="list_pg" object="<?=$current_object;?>" href="javascript:void(0)"><?=$total_pages;?></a></li>
				<?
			}
			...
			if($current_page < $total_pages){ 
				?>
				<li>
					<a href="javascript:void(0)" rel="<?=$current_page+1;?>" object="<?=$current_object;?>" class="list_pg pag_nav">
						<span>
							<span>Suivant</span>
						</span>
					</a>
